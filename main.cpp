...
void Timer(int i)
{
  glutPostRedisplay();   ///Mark the normal plane of current window as needing to be redisplayed
    glutTimerFunc(10, Timer, 0);///i=1 initially;
}
...
void car()
{
    ...
    {
        float x=wx+sin(i)*7;  ///behind wheel
        float y=wy+cos(i)*7;
        glVertex2f(x,y);
    }
    glEnd();
    ...
    for(float i=1.0;i<360.0;i++)
    {
        float x=wx1+sin(i)*7;  ///front wheel
        float y=wy1+cos(i)*7;
        glVertex2f(x,y);
    }
    glEnd();
...
void myinit(void)
{
	...
	///Drawing a geometric shape of 4 vertices that is backgroung
    glBegin(GL_QUADS);
    glColor3f(0.5,0.3,0);
    glVertex2f(0,0);
    glColor3f(0.5,0.3,0);
    ...
    glRasterPos2f( 400 , 900 );      ///specify the raster position for pixel operations
    for(int i = 0; c[i] !='\0'; i++)
        glutBitmapCharacter(GLUT_BITMAP_HELVETICA_18, c[i]);    ///glutBitmapCharacter renders a bitmap character using OpenGL.
                                                                ///GLUT_BITMAP_HELVETICA_18 is the font type
...
///argv is a pointer to an array and
///argc says how large is this array

///The arguments argc and argv of main is used as a way to send arguments to a program i.e,they are command line arguments
int main(int argc,char** argv)
{
    ///initializes the freeglut
    glutInit(&argc,argv);
    glutInitDisplayMode(GLUT_SINGLE|GLUT_RGB);   ///GLUT_SINGLE - single buffered window i.e, there is only a single frame buffer to be drawn
    glutInitWindowPosition(0,0);
    glutInitWindowSize(1500,1500);
    window=glutCreateWindow("car Animation");

    ///glutDisplayFunc is called whenever your window is redrawn i.e, whenever some actions are performed it redisplays the window
    glutDisplayFunc(myinit);

    ///glutMouseFunc is used for handling mouse click events
	glutMouseFunc(mouse);
